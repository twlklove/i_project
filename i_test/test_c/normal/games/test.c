...
{ 
	...
	//假设左边为墙，直接return 
	if(map[ly][lx] == 1){ return; } 
	//假设左边为箱子 
	if(map[ly][lx] == 3){ 
		//判断箱子左边是否为墙 
		if(map[ly][lx - 1] == 1){ return; } 
		//判断箱子左边是否为球 
		if(map[ly][lx - 1] == 4){ 
			//将箱子左边内容赋值为5★ 
			map[ly][lx - 1] = 5; 
...
{ 
	...
	dx = x; dy = y + 1; 
	//下方为已完成方块 
	if(map[dy][dx] == 5){ return; } 
	//假设下方为墙，直接return 
	if(map[dy][dx] == 1){ return; } 
	//假设下方为箱子 
	if(map[dy][dx] == 3){ 
		//判断箱子下方是否为墙 
		if(map[dy + 1][dx] == 1){ return; } 
		//判断箱子下方是否为球 
		if(map[dy + 1][dx] == 4){ 
			//将箱子下面内容赋值为5★ 
			map[dy + 1][dx] = 5; map[dy][dx] = 0; 
			//箱子的数目减1 
			boxs--; 
		}else{ 
			//移动箱子 
...
void moveRight(){ 
	...
	rx = x + 1; ry = y; 
	//右边为已完成方块 
	if(map[ry][rx] == 5){ return; } 
	//假设右边为墙，直接return 
	if(map[ry][rx] == 1){ return; } 
	//假设右边为箱子 
	if(map[ry][rx] == 3){ 
		//判断箱子右边是否为墙 
		if(map[ry][rx + 1] == 1){ return; } 
		//判断箱子左边是否为球 
		if(map[ry][rx + 1] == 4){ 
			//将箱子右边内容赋值为5★ 
			map[ry][rx + 1] = 5; map[ry][rx] = 0; 
			//箱子的数目减1 
			boxs--; 
		}else{ 
			//移动箱子 
