...
bool currentProcessAffinity(std::vector<unsigned int>& runningCPUVector)
{
     ...
     CPU_ZERO(&cpuSet);
     // 得到指定进程ID绑定到哪个CPU
     int ret = sched_getaffinity(0, sizeof(cpuSet),  &cpuSet);// 0代表当前进程
...
bool setCurrentProcessAffinity(const std::vector<unsigned int>& needBindCPUVector)
{
    ...
    // 将指定进程ID绑定到CPU
    int ret = sched_setaffinity(0, sizeof(cpuSet), &cpuSet); // 0代表当前进程
    if (ret < 0)
    {
        return false;
    }
...
int main()
{
     ...
     std::vector<unsigned int> needBindCPUVector {0, 2};  // bind to core 0 and 2
     if (!setCurrentProcessAffinity(needBindCPUVector))  // set process affinity
     {
     ...
     runningCPUVector.clear();
     if (!currentProcessAffinity(runningCPUVector))  // get process affinity
     {
         printf("Get current process was bound witch CPU failed\n");
         return 1;
     }
     ...
     needBindCPUVector.push_back(1);  // bind to core 1
     ...
     runningCPUVector.clear();
     if (!currentThreadAffinity(runningCPUVector))  // get thread affinity
     {
         printf("Get current thread was bound witch CPU failed\n");
         return 1;
     }
